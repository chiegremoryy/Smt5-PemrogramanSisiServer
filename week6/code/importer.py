...
os.environ['DJANGO_SETTINGS_MODULE'] = 'simplems.settings'
import django
django.setup()
...
with open('./core/csv_data/user-data.csv') as csvfile:
...
with open('./core/csv_data/course-data.csv') as csvfile:
    reader = csv.DictReader(csvfile)
    for num,row in enumerate(reader):           
...
with open('./core/csv_data/member-data.csv') as csvfile:
    reader = csv.DictReader(csvfile)
    for num, row in enumerate(reader):
